  ...
  const normalizeItems = (items = []) => {
    return items.map((item) => {
      ...
      const sellerEarnings =
        item.seller_earnings != null
          ? Number(item.seller_earnings)
          : lineTotal;
    ...
    const detailedOrders = await Promise.all(
      (ordersData.orders || []).map(async (order) => {
        ...
        return {
          ...
          total_price: Number(mergedOrder.total_price || 0),
          delivery_charge: Number(mergedOrder.delivery_charge || 0),
          discount_amount: Number(mergedOrder.discount_amount || 0),
