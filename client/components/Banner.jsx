...
export default function Banner() {
  ...
  const [isPlusMember, setIsPlusMember] = React.useState(false)
    ...
    const checkPlusStatus = async () => {
      try {
        ...
        const active =
          user?.is_plus_member &&
          user?.plus_expiry &&
          new Date(user.plus_expiry) > new Date()
  ...
  const handleGoToPlusPage = () => {
    router.push('/plusmember')
  }
      ...
      <div className="relative mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6">
        <div className="flex min-w-0 items-center gap-3 text-sm text-white/95">
          ...
          <p className="truncate font-medium tracking-[0.01em] text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.35)]">
            <span className="font-semibold text-white">Plus Member Feature:</span>{' '}
            Earn more points on eligible delivered orders, get welcome bonus points,
            unlock reward promos, and enjoy premium member status.
          </p>
          ...
          <span className="hidden items-center gap-1.5 rounded-full border border-white/30 bg-white/10 px-3 py-1 text-xs font-semibold tracking-[0.08em] text-white backdrop-blur sm:flex">
            <Star size={12} />
            {loading ? 'CHECKING' : isPlusMember ? 'PLUS ACTIVE' : 'JOIN NOW'}
          </span>
        </div>

        <div className="flex shrink-0 items-center gap-2">
          <button
            onClick={handleGoToPlusPage}
            className="
