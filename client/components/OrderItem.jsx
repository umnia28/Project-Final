...
const getSellerBadgeText = (item) => {
  if (item.cancelled_by === "admin") return "Admin Cancelled";
      ...
      <tr className="align-top">
        <td className="py-6 text-left">
          <div className="flex flex-col gap-5">
            {order.items?.map((item) => {
              ...
              const unitPrice = Number(item.price || 0); // discounted unit price
              const itemDiscount = Number(item.discount_amount || 0); // total product discount for line
              const lineTotal =
              ...
              const originalLineTotal =
                item.original_line_total != null
                  ? Number(item.original_line_total)
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex flex-col gap-3">
                      ...
                      {itemDiscount > 0 ? (
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-sm font-medium text-slate-700">
                            Line Total After Discount: {currency}
                    ...
                    {(item.cancel_reason || getCancelledMetaText(item)) && (
                      <div className="space-y-1">
                        <p className="text-xs text-slate-500">
                          {getCancelledMetaText(item)}
                        </p>
                        {item.cancel_reason && (
                          <p className="text-xs text-slate-500">
        ...
        <td className="max-md:hidden py-6 text-left">
          <div className="rounded-2xl border border-white/70 bg-white/70 p-4 text-sm text-slate-600 shadow-sm backdrop-blur-md">
            ...
            ) : (order.shipping_address || order.city || order.shipping_state || order.zip_code || order.country) ? (
              <div className="space-y-1">
                {order.shipping_address && <p>{order.shipping_address}</p>}
                <p>
                  {[order.city, order.shipping_state, order.zip_code, order.country]
                    .filter(Boolean)
                    .join(", ")}
                </p>
              </div>
          ...
          <div className="rounded-2xl border border-white/70 bg-white/70 p-4 shadow-sm backdrop-blur-md">
            ...
            <div className="flex flex-col gap-2 text-sm">
              ...
              <Link
                ...
                className="mt-3 inline-flex w-fit items-center justify-center rounded-full bg-gradient-to-r from-[#d8c3a5] via-[#a78bdb] to-[#7fb6ea] px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_22px_rgba(167,139,219,0.16)] transition hover:scale-[1.02]"
              >
                Track Order
              </Link>
            </div>
