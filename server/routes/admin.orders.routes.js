...
async function restockOrderItems(client, orderId) {
  ...
  for (const r of rows) {
    await client.query(
      ...
      WHERE product_id = $2
      `,
      [Number(r.qty || 0), r.product_id]
    );
  }
}

/* =========================
   HELPER: calculate proportional live item shares
========================= */
  ...
  async (req, res) => {
    try {
      ...
      const orders = ordersRes.rows.map((order) => ({
        ...order,
        refund_amount: Number(order.refunded_amount || 0),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (pendingItems > 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({
          message: "Cannot assign delivery man until all items are processed by sellers",
        });
      }
      ...
      await client.query(
        ...
        VALUES ($1, $2)
        `,
        [delivery_man_id, `Order #${orderId} has been assigned to you.`]
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      const activeItems = allItems.filter(
        (item) => String(item.seller_status || "").toLowerCase() !== "cancelled"
      );
      ...
      const shares = calculateItemShares(allItems, Number(order.total_price || 0));
      ...
      for (const item of shares) {
        if (String(item.seller_status || "").toLowerCase() === "cancelled") {
          continue;
        }

        const itemShare = Math.round(Number(item.share || 0) * 100) / 100;
        totalReduction += itemShare;

        const itemRefund = isPaidLike ? itemShare : 0;
        refundAdd += itemRefund;

        const refundStatus = itemRefund > 0 ? "refunded" : "not_refunded";
      ...
      const newTotal = Math.max(
        0,
        Math.round((Number(order.total_price || 0) - totalReduction) * 100) / 100
      );

      const newRefund =
        Math.round((Number(order.refunded_amount || 0) + refundAdd) * 100) / 100;
      ...
      await client.query(
        ...
        WHERE order_id = $5
        `,
        [newTotal, newRefund, reason, newPaymentStatus, orderId]
      );
...
// async function restockOrderItems(client, orderId) {
//   const { rows } = await client.query(
//     `SELECT product_id, qty FROM order_item WHERE order_id = $1::bigint`,
//     [orderId]
//   );
...
//       `
//       UPDATE product
//       SET product_count = COALESCE(product_count, 0) + $1::integer
...
//         `
//         UPDATE "order"
//         SET delivery_man_id = $1::bigint
//         WHERE order_id = $2::bigint
//         `,
//         [delivery_man_id, orderId]
//       );
...
//         return res
//           .status(400)
//           .json({ message: "Cancelled/refunded order cannot be delivered" });
//       }
...
//         `
//         INSERT INTO order_status (order_id, status_type, updated_by)
//         VALUES ($1::bigint, 'out_for_delivery', $2::bigint)
//         `,
//         [orderId, adminId]
//       );
...
//         `
//         INSERT INTO order_status (order_id, status_type, updated_by)
//         VALUES ($1::bigint, 'delivered', $2::bigint)
//         `,
//         [orderId, adminId]
//       );
...
//         const itemShare = Number(item.share || 0);
//         totalReduction += itemShare;
...
//             refund_status = CASE
//               WHEN $3::numeric > 0 THEN 'refunded'
//               ELSE COALESCE(refund_status, 'not_refunded')
//             END,
//             refunded_at = CASE
//               WHEN $3::numeric > 0 THEN NOW()
//               ELSE refunded_at
//             END
//           WHERE order_item_id = $1::bigint
//           `,
//           [item.order_item_id, reason, itemRefund]
//         );
//       }
...
//           reason_for_cancellation = $3::text,
//           payment_status = CASE
//             WHEN $2::numeric > 0 AND $1::numeric <= 0 THEN 'refunded'
//             WHEN $2::numeric > 0
//               AND LOWER(COALESCE(payment_status, '')) IN ('paid', 'partially_refunded', 'refunded')
//               THEN 'partially_refunded'
//             ELSE payment_status
//           END
//         WHERE order_id = $4::bigint
//         `,
//         [newTotal, newRefund, reason, orderId]
//       );

//       await client.query(
//         `
//         INSERT INTO order_status(order_id, status_type, updated_by)
//         VALUES ($1::bigint, 'cancelled', $2::bigint)
//         `,
//         [orderId, adminId]
//       );
...
// //   for (const r of rows) {
// //     await client.query(
// //       `UPDATE product SET product_count = COALESCE(product_count, 0) + $1 WHERE product_id=$2`,
// //       [r.qty, r.product_id]
// //     );
// //   }
// // }
...
// //         (Number(order.refunded_amount || 0) + refundAdd) * 100
// //       ) / 100;
...
// // //         return {
// // //           ...order,
// // //           refund_amount: Number(order.refunded_amount || 0), // use order table as source of truth
// // //           items,
// // //         };
// // //       });
