...
router.get("/preview/:store_id", verifyToken, requireRole("admin"), async (req, res) => {
  ...
  try {
    ...
    const previewRes = await pool.query(
      ...
      WHERE p.store_id = $1
        AND oi.seller_status = 'confirmed'
        AND oi.delivery_status = 'delivered'
        AND oi.cancelled_by IS NULL
        AND COALESCE(oi.refund_status, 'not_refunded') = 'not_refunded'
    ...
    return res.json({
      ...
      pending_amount: Number(previewRes.rows[0].pending_amount || 0),
    });
  } catch (error) {
...
router.post("/create", verifyToken, requireRole("admin"), async (req, res) => {
  ...
  try {
    ...
    const storeRes = await client.query(
      `SELECT user_id, store_name FROM store WHERE store_id = $1`,
      [store_id]
    );
    ...
    if (itemsRes.rows.length === 0) {
      ...
      return res.status(400).json({
        success: false,
        message: "No payout-eligible delivered items found for this store",
      });
    }
